...
- herdr protocol 22. Warn on mismatch, never refuse to run.
...
- [ ] **Step 1:** Write tests against `test/fixtures/schema-protocol-22.json`: 103 methods; `pane.read` requires `pane_id` and `source`; `ReadSource` `$ref` resolves to its four-value enum.
...
- `(herdr-call METHOD)` — interactive, completes over all 103 methods, prompts each param from its schema.
